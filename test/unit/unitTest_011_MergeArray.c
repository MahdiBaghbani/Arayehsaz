/** test/unitTest_011_merge_array.c
 *
...
void test_merge_array_empty_arayeh_step_1(void)
{
    ...
    for (size_t index = start_index; index < arayeh_size; index += step) {
        ...
        TEST_ASSERT_EQUAL_INT(c_generic_array[index],
...
void test_merge_array_empty_arayeh_step_5(void)
{
    ...
    for (size_t arayeh_index = start_index, c_array_index = 0; arayeh_index < arayeh_size;
        ...
        // assert array is successfully merged into arayeh.
        TEST_ASSERT_EQUAL_INT(c_generic_array[c_array_index],
                              private_properties->array.int_pointer[arayeh_index]);
        TEST_ASSERT_EQUAL_CHAR(1, is_arayeh_map_filled(test_case, arayeh_index));
    }

    for (size_t index = start_index; index < arayeh_size; index++) {
        if (index % 5 != 0) {
            // assert that arayeh is empty in this cells.
            TEST_ASSERT_EQUAL_CHAR(0, is_arayeh_map_filled(test_case, index));
        }
    }
...
void test_general_ON_method_specific_merge_array_extension_OFF(void)
{
    ...
    arayeh_size_settings new_size_settings = {.extend_add          = AA_ARAYEH_ON,
                                              ...
                                              .extend_merge_array  = AA_ARAYEH_OFF};
...
void test_merge_error_bad_starting_point(void)
{
    ...
    // merge array.
    state = test_case->merge_array(test_case, -5, 1, 5, &c_generic_array);
...
void test_merge_array_error_wrong_step(void)
{
    ...
    // merge array.
    state = test_case->merge_array(test_case, 0, 0, 5, &c_generic_array);

    // fill was successful.
    TEST_ASSERT_EQUAL_INT(AA_ARAYEH_WRONG_STEP, state);

    // merge array.
    state = test_case->merge_array(test_case, 0, -5, 5, &c_generic_array);
